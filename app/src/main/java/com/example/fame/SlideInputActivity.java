...
public class SlideInputActivity extends AppCompatActivity {
            ...
            public void onClick(View v) {
                if(wordText.getText().toString().equals(editText.getText().toString())){
                    if(cnt>1) {
                        ...
                        //ActivityCompat.finishAffinity(SlideInputActivity.this);//모든 엑티비티 종료
                        finish();
                    }
                }
            }
    ...
    public int inputcount(){
        ...
        SQLiteDatabase db=DBHelper.getInstance(SlideInputActivity.this).getReadableDatabase();
        String sql = "SELECT * FROM SlideCategory WHERE _id="+id;
